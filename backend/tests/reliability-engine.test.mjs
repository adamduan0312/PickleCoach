...
const coachRow = (overrides = {}) => {
  const canonical = buildCanonicalReliabilityMetrics(baseRaw());
...
describe('reliabilityEngine', () => {
  it('denominator uses booking_baseline_total + smoothing_k', () => {
    const c = buildCanonicalReliabilityMetrics(baseRaw({ booking_baseline_recent: 4, booking_baseline_decayed: 1 }));
  ...
  it('persisted totals match scoring inputs (coach)', () => {
    ...
    assert.equal(c.coach_cancels_non_late_total, 2);
